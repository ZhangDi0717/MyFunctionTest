...
/**
 * Created by zhangdi45 on 2022/9/15.
 */
public class Camera2Proxy {
    ...
    private int mCameraId = 3; // 要打开的摄像头ID
    ...
    private void test(Size[] outputSizes) {
        for (int i = 0; i < outputSizes.length; i++) {
            Log.d(TAG, "outputSizes: " + outputSizes[i].getWidth() + "*" + outputSizes[i].getHeight());
        }
    }
    ...
    private int getRotation() {
        ...
        Log.d(TAG, "getRotation: displayRotation = "+displayRotation);
        int sensorOrientation = mCameraCharacteristics.get(CameraCharacteristics.SENSOR_ORIENTATION);
        Log.d(TAG, "getRotation: sensorOrientation = "+sensorOrientation);
        mDisplayRotate = (displayRotation + sensorOrientation + 270) % 360;
        Log.d(TAG, "getRotation: mDisplayRotate = "+mDisplayRotate);
